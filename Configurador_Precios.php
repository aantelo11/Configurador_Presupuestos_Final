...
<section>
    <form action="" method="post" id="formulario">
        <?php if ($resultado->num_rows > 0): ?>
            <h3 class="form-title textos-estilos">Selecciona los servicios que desea contratar:</h3>
            <article class="servicios-estilo">
                <?php foreach ($resultado as $servicio): ?>
                    <div class="servicio-estilos" id="tarjeta-<?= htmlspecialchars($servicio['id']) ?>">
                        <img src="imagenes/tick.svg" class="no-seleccionado" alt="seleccionado">
                        <img class="imagen-estilos" src="<?= htmlspecialchars($servicio['imagen_ruta']) ?>" alt="">
                        <div class="textos-div-stilos">
                            <p class="textos-estilos tipo-estilos"><?= htmlspecialchars($servicio['tipo']) ?></p>
                            <p class="textos-estilos nombre-servicio-estilos"><?= htmlspecialchars($servicio['nombre']) ?></p>
                        </div>
                        <button type="button" class="textos-estilos precio-estilos" data-id="<?= htmlspecialchars($servicio['id']) ?>">Contratar</button>
                        <input type="checkbox" name="servicios[]" value="<?= htmlspecialchars($servicio['id']) ?>" class="checkbox-servicio" hidden>
                    </div>
                <?php endforeach; ?>
                ...
                <div class="formulario-container textos-estilos">
                    ...
                    <div class="grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="nombre" name="nombre" placeholder="Nombre" pattern="^^[A-Za-zÀ-ÿ\u00f1\u00d1' -]+$"
                                title="Solo se permiten letras y espacios" required>
                        </div>

                        <div class="form-group">
                            <label for="apellidos">Apellidos</label>
                            <input type="text" id="apellidos" name="apellidos" placeholder="Apellido" pattern="^[A-Za-zÀ-ÿ\u00f1\u00d1' -]+$"
                                title="Solo se permiten letras y espacios" required>
                        </div>

                        <div class="form-group">
                            <label for="telefono">Teléfono</label>
                            <input type="tel" id="telefono" name="telefono" placeholder="+34 123456789" pattern="^\+?[0-9\s\-\(\)]{7,20}$" title="Ingrese un número de teléfono válido. Puede incluir el prefijo internacional (+), espacios o guiones." required>
                        </div>
