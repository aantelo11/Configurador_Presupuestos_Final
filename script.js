...
function botonPulsado(evento) {
    ...
    const servicioTarjeta = boton.dataset.id;
    const tarjeta = document.getElementById("tarjeta-" + servicioTarjeta);
    const seleccionado = tarjeta.querySelector('img[alt="seleccionado"]');

    evento.target.classList.toggle("activo");

    seleccionado.classList.toggle("no-seleccionado");
